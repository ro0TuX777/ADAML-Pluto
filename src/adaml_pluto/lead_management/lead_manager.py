...
class LeadManager:
    ...
    def list_leads(
        ...
        if min_score is not None:
            filtered_leads = [l for l in filtered_leads if l.score >= min_score]
        
        if company:
            filtered_leads = [
                l for l in filtered_leads 
                if l.company and company.lower() in l.company.lower()
            ]
    ...
    def search_leads(self, query: str) -> List[Lead]:
        ...
        for lead in self.leads.values():
            if (
                query_lower in lead.first_name.lower() or
                query_lower in lead.last_name.lower() or
                query_lower in lead.email.lower() or
                (lead.company and query_lower in lead.company.lower())
            ):
                results.append(lead)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for status in LeadStatus:
            by_status[status.value] = len([
                l for l in self.leads.values() if l.status == status
            ])
        
        avg_score = (
            sum(l.score for l in self.leads.values()) / total
            if total > 0 else 0
        )
